...
from .exp import  get_bigbird_mask, get_fibottention_mask
...
def extractK(x):
    ...
    # Reshape x to (batch_size * num_patches, 768) for batch-wise processing
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        #self.scale = ((1-0.4)** -2 )*(self.head_dim ** -0.5)
    ...
    def forward(self, x):
        ...
        if self.fused_attn:
            ...
            '''
            Dominick: This is where the desired attention mask should be applied
            '''
            ## Bigbird masked attention
            # new_mask_attn = get_bigbird_mask(q, k)

            ## Fibottention
            cache_key = f'{self.block_idx}_{q.shape}'
            if cache_key not in self.cache:
                new_mask_attn = get_fibottention_mask(q, self.block_idx, True)
                self.cache[cache_key] = new_mask_attn
            else:
                new_mask_attn = self.cache[cache_key]
