...
def evaluate_results(data_loader, model, device, mask_q, mask_k, mask_attn):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        if result_tensor is None:
            result_tensor = output.detach().clone()
        else:
            result_tensor = torch.cat([result_tensor, output.detach().clone()], dim=0)

        if target_tensor is None:
            target_tensor = target.detach().clone()
        else:
            target_tensor = torch.cat([target_tensor, target.detach().clone()], dim=0)
