...
class TestMeter(object):
    ...
    def finalize_metrics(self, ks=(1, 5)):
        ...
        if self.multi_label:
            ...
            # log mean class accuracy (mCA)
            
            conf_matrix, class_accuracies, mean_perclass_accuracy = metrics.mean_class_accuracies(self.video_preds, self.video_labels, self.num_cls)
            mean_perclass_accuracy = mean_perclass_accuracy * 100.0
            
            # save confusion matrix
            conf_matrix = conf_matrix.numpy()

            # logger.info(f'Saving test set confusion matrix to: {self.cfg.OUTPUT_DIR}/testset_confusion_matrix.npy')
            # np.save(f'{self.cfg.OUTPUT_DIR}/testset_confusion_matrix.npy', conf_matrix)

            self.stats['mCA'] = f'{mean_perclass_accuracy:.2f}'    
