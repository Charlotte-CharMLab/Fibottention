...
from timesformer.models.mask import get_mask_attn
...
class Attention(nn.Module):
    mask_plotted = False # Added by me
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., with_qkv=True,depth_id=0):
        super().__init__()
    ...
    def forward(self, x,s_mask,t_mask):
        B, N, C = x.shape
        if self.with_qkv:
        ...
        if attn_shape[2] == 197 :
            if self.depth_id not in self.cache:
                new_mask_attn = get_mask_attn_wythoff(q, self.depth_id,True)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0.1, act_layer=nn.GELU, norm_layer=nn.LayerNorm, attention_type='divided_space_time',depth_id=0):
        super().__init__()
        ...
        self.depth_id = depth_id
        self.attn = Attention(
           dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,depth_id=depth_id)
    ...
    def forward(self, x, B, T, W,s_mask,t_mask):
        num_spatial_tokens = (x.size(1) - 1) // T
        H = num_spatial_tokens // W

        if self.attention_type in ['space_only', 'joint_space_time']:
            x = x + self.drop_path(self.attn(self.norm1(x),s_mask,t_mask))
            ...
            res_temporal = self.drop_path(self.temporal_attn(self.temporal_norm1(xt),s_mask,t_mask))
            ...
            res_spatial = self.drop_path(self.attn(self.norm1(xs),s_mask,t_mask))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, attention_type=self.attention_type , depth_id=i)
    ...
    def forward_features(self, x,s_mask, t_mask):
        B = x.shape[0]
    ...
    def forward(self, x,s_mask,t_mask):
        x = self.forward_features(x,s_mask,t_mask)
        x = self.head(x)
        return x
