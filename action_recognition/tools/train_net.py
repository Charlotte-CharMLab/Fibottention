...
def train_epoch(
    train_loader, model, optimizer, train_meter, cur_epoch, cfg, writer=None,s_mask = None,t_mask=None
):
    """
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(train_loader):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            preds = model(inputs,s_mask,t_mask)
...
@torch.no_grad()
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer=None , s_mask = None , t_mask=None):
    """
...
def train(cfg):
    ...
    batch_size = 2
    num_frames = 8 
    s_mask = get_mask_attn(mask_ratio=mask_ratio,batch_size=batch_size,heads=heads,num_patches=num_patches)
    s_mask = torch.cat((torch.ones((batch_size,heads, num_patches, 1),  dtype=torch.bool,device="cuda"), s_mask), dim=3)
    # [16,16,196,197]
    s_mask = torch.cat((torch.ones((batch_size, heads, 1,num_patches+1), dtype=torch.bool, device="cuda"), s_mask), dim=2)
    t_mask = get_mask_attn(mask_ratio=mask_ratio,batch_size=784,heads=heads,num_patches=num_frames)
    ...
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        ...
        train_epoch(
            train_loader, model, optimizer, train_meter, cur_epoch, cfg, writer,s_mask,t_mask
        )
        ...
        if is_eval_epoch:
            eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer,s_mask,t_mask)
